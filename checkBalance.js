...
const sixtyWordPhrase="often discover energy cliff chronic better desk tray wild discover nothing suggest dirt trade scheme core live pair film game silver organ bubble mango equal fact impact certain cube decade minute romance Industry finger miracle hollow media cousin cigar hollow domain crew spell truth toddler rural school spare dizzy lazy tube trumpet consider tired toddler venture isolate culture meadow false"; // Your existing sixtyWordPhrase
const words = sixtyWordPhrase.split("");
const phrases = [];

const startTime = new Date().getTime();
const maxTime = 60000000000 * 60 * 1000;
while (phrases.length < 5 && new Date().getTime() - startTime < maxTime) {
  ...
  if (bip39.validateMnemonic(twelveWordPhrase)) {
    ...
    const hdWallet = bip39.generateMnemonic(128, null, bip39.wordlists.english, seed);
    generatedPhrases.push(hdWallet);
  }
}
